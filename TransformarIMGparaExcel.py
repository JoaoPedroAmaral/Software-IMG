...
def image_to_excel_cmyk(image_path, excel_path):
    # Leitura da imagem
...
def image_to_excelRGB(image_path, excel_path):
    # Leitura da imagem
    img = cv2.imread(image_path)
    ...
    for row, row_pixels in enumerate(img):
        for col, pixel in enumerate(row_pixels):
...
def image_to_matrix_gray(image_path):
    try:
            # Abre a imagem
...
def save_matrix_to_excel_gray(matrix, excel_filename):
    try:
...
def select_image():
        # Pega a imagem
        file_path = "IMG\madacasca.jpg"

        # Verifica se a imagem foi selecionada
        if file_path:
            image_gray = image_to_matrix_gray(file_path)
            image_to_excelRGB(file_path,"saidaRGB.xlsx")
            image_to_excel_cmyk(file_path,"saidaCMYK.xlsx")
            if image_gray is not None:
                save_matrix_to_excel_gray(image_gray, "saidaGray.xlsx")
